...
docker:
	$(NAIS) validate
	$(DOCKER) build --pull -t $(REGISTRY)/tpregisteret -t $(REGISTRY)/tpregisteret:$(VERSION) .

docker-push:
	$(DOCKER) push $(REGISTRY)/tpregisteret:$(VERSION)
...
manifest:
	nais upload --app tpregisteret -v $(VERSION)